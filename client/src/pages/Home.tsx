  ...
  return <div className="welcome-app"><EventHeader /><main className="container welcome-main" id="welcome">
    <section className="compact-welcome" aria-labelledby="welcome-title">
      <div className="compact-welcome__copy">
        ...
        <p>One simple Welcome Day experience: choose a studio, answer three quick questions, and see your result on the live board.</p>
        <ol className="welcome-steps" aria-label="How the Welcome Day works"><li><b>1</b><span><strong>Start</strong>Make your record</span></li><li><b>2</b><span><strong>Choose</strong>Pick one ICT studio</span></li><li><b>3</b><span><strong>Finish</strong>See your result live</span></li></ol>
      ...
      <section className="join-station compact-join" aria-label="Start your TFA quiz record"><div className="join-station__head"><span>START HERE</span><h2>{accessCode ? "Your ICT route is ready." : "Create your record."}</h2><p>{accessCode ? "Your progress is saved. Pick your next studio below." : "Name and grade are all you need. A class code is optional."}</p></div>{accessCode ? <div className="join-station__ready"><BookOpenCheck size={25} /><div><strong>{passport.data?.participant.displayName ?? "Quiz record ready"}</strong><span>{completedCount} of 5 ICT studios completed · {passport.data?.totalPoints ?? 0} points</span></div><Button variant="ghost" onClick={() => setLocation("/passport")}>My record <ArrowRight size={16} /></Button></div> : <div className="join-station__form"><label><span>Your name</span><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Your name" maxLength={80} /></label><label className="grade-picker"><span>Grade group</span><button type="button" className="grade-picker__trigger" aria-haspopup="listbox" aria-expanded={gradePickerOpen} onClick={() => setGradePickerOpen(open => !open)}><span>{GRADE_GROUPS.find(group => group.value === gradeBand)?.label ?? "Choose grade"}</span><ChevronDown size={17} /></button>{gradePickerOpen && <span className="grade-picker__options" role="listbox" aria-label="Choose grade group">{GRADE_GROUPS.map(group => <button type="button" role="option" aria-selected={gradeBand === group.value} key={group.value} onClick={() => { setGradeBand(group.value); setGradePickerOpen(false); }}>{group.label}</button>)}</span>}</label><label className="join-station__class-field"><span>Class code <em>optional</em></span><Input value={classLabel} onChange={event => setClassLabel(event.target.value)} placeholder="Example: 7F" maxLength={40} /><small>Write 7F, 10A, or add Girl/Boy if you wish.</small></label><Button className="action-button" onClick={start} disabled={join.isPending}>{join.isPending ? "Starting…" : <>Start my ICT route <ArrowRight size={17} /></>}</Button></div>}</section>
    </section>

    <section className="ict-studio-hub" id="ict-studios" aria-labelledby="ict-studios-title">
      <header><p className="eyebrow">YOUR NEXT STEP</p><h2 id="ict-studios-title">Choose <em>one</em> ICT studio.</h2><p>Each studio has three short, grade-aware questions. Start anywhere—there is no fixed order.</p></header>
      ...
      <aside className="studio-hub__tip"><CircleHelp size={20} /><p><strong>What is the purpose?</strong> Try useful ICT skills from your classroom displays, learn something new, and celebrate your result with your class.</p><Button variant="ghost" onClick={() => setLocation("/live")}>Watch results <Radio size={15} /></Button></aside>
    </section>

    <details className="welcome-extras"><summary><span>More ICT fun for Welcome Day</span><small>Open Tech Wheel, Class of 2040, Human or AI, photo ideas, and resources</small><ChevronDown size={18} /></summary><div className="welcome-extras__content">
