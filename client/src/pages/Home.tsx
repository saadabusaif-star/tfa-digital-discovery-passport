...
import { ArrowRight, BookOpenCheck, Camera, Cpu, Gamepad2, Keyboard, Loader2, Radio, RotateCw, Sparkles, Trophy } from "lucide-react";
...
export default function Home() {
  ...
  const [scannedClassId] = useState<number | undefined>(() => {
  ...
  const classSession = trpc.event.classSession.useQuery({ classGroupId: scannedClassId ?? 0 }, { enabled: Boolean(scannedClassId) });
  ...
  const start = () => { if (!displayName.trim() || !gradeBand || !classSession.data) return toast.error("Scan your teacher's QR code, then enter your name and grade group."); join.mutate({ displayName, gradeBand, eventSection: classSession.data.eventSection, classGroupId: classSession.data.id }); };
  ...
  return <div className="welcome-app"><EventHeader /><main className="container welcome-main" id="welcome">
    <section className="welcome-intro">
      ...
      <section className="join-station" aria-label="Start your TFA quiz record"><div className="join-station__head"><span>YOUR TFA DISCOVERY PASSPORT</span><h2>{accessCode ? "Your ICT route is ready." : scannedClassId ? "Start your ICT route." : "Scan your teacher QR."}</h2><p>{accessCode ? "Choose any ICT studio. Your completed challenges will stay in your personal record." : scannedClassId ? "Your teacher has opened this class session. Enter your name and grade group to begin." : "Your teacher selects the class privately, then shows one QR code. Scan it with your tablet or phone to join automatically."}</p></div>{accessCode ? <div className="join-station__ready"><BookOpenCheck size={25} /><div><strong>{passport.data?.participant.displayName ?? "Quiz record ready"}</strong><span>{completedCount} of 5 ICT studios completed · {passport.data?.totalPoints ?? 0} points</span></div><Button variant="ghost" onClick={() => setLocation("/passport")}>View record <ArrowRight size={16} /></Button></div> : scannedClassId ? classSession.isLoading ? <div className="join-station__ready"><Loader2 className="animate-spin" size={22} /><span>Opening your teacher session…</span></div> : classSession.isError ? <div className="join-station__ready"><span>This class QR is no longer active. Ask your teacher to show the current code.</span></div> : <div className="join-station__form"><label><span>Display name</span><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Your name" maxLength={80} /></label><label><span>Grade group</span><select value={gradeBand} onChange={event => setGradeBand(event.target.value as typeof gradeBand)}><option value="">Choose grade</option><option value="6-7">Grades 6–7</option><option value="8-9">Grades 8–9</option><option value="10-12">Grades 10–12</option></select></label><Button className="action-button" onClick={start} disabled={join.isPending}>{join.isPending ? "Starting…" : <>Start ICT route <ArrowRight size={17} /></>}</Button></div> : <div className="join-station__ready join-station__waiting"><BookOpenCheck size={25} /><div><strong>Waiting for your class QR.</strong><span>Students do not choose a teacher or class.</span><button type="button" onClick={() => setLocation("/staff")}>Teacher? Open the QR launcher <ArrowRight size={14} /></button></div></div>}</section>
    </section>
