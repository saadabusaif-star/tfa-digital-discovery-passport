...
import { ArrowRight, BookOpenCheck, Camera, ChevronDown, Cpu, Gamepad2, Keyboard, Radio, RotateCw, Sparkles, Trophy } from "lucide-react";
...
export default function Home() {
  ...
  const start = () => { if (!displayName.trim() || !gradeBand || !classLabel.trim()) return toast.error("Enter your name, grade group, and section and class such as Boy 7F."); join.mutate({ displayName, gradeBand, classLabel }); };
  ...
  return <div className="welcome-app"><EventHeader /><main className="container welcome-main" id="welcome">
    <section className="welcome-intro">
      ...
      <section className="join-station" aria-label="Start your TFA quiz record"><div className="join-station__head"><span>YOUR TFA DISCOVERY PASSPORT</span><h2>{accessCode ? "Your ICT route is ready." : "Create your ICT route."}</h2><p>{accessCode ? "Choose any ICT studio. Your completed challenges will stay in your personal record." : "Enter your name, grade group, and class. For example: Boy 7F or Girl 10A."}</p></div>{accessCode ? <div className="join-station__ready"><BookOpenCheck size={25} /><div><strong>{passport.data?.participant.displayName ?? "Quiz record ready"}</strong><span>{completedCount} of 5 ICT studios completed · {passport.data?.totalPoints ?? 0} points</span></div><Button variant="ghost" onClick={() => setLocation("/passport")}>View record <ArrowRight size={16} /></Button></div> : <div className="join-station__form"><label><span>Display name</span><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Your name" maxLength={80} /></label><label className="grade-picker"><span>Grade group</span><button type="button" className="grade-picker__trigger" aria-haspopup="listbox" aria-expanded={gradePickerOpen} onClick={() => setGradePickerOpen(open => !open)}><span>{GRADE_GROUPS.find(group => group.value === gradeBand)?.label ?? "Choose grade"}</span><ChevronDown size={17} /></button>{gradePickerOpen && <span className="grade-picker__options" role="listbox" aria-label="Choose grade group">{GRADE_GROUPS.map(group => <button type="button" role="option" aria-selected={gradeBand === group.value} key={group.value} onClick={() => { setGradeBand(group.value); setGradePickerOpen(false); }}>{group.label}</button>)}</span>}</label><label className="join-station__class-field"><span>Section and class</span><Input value={classLabel} onChange={event => setClassLabel(event.target.value)} placeholder="Boy 7F" maxLength={40} /><small>Write Boy or Girl, then your class. Example: Boy 7F.</small></label><Button className="action-button" onClick={start} disabled={join.isPending}>{join.isPending ? "Starting…" : <>Start ICT route <ArrowRight size={17} /></>}</Button></div>}</section>
    </section>
