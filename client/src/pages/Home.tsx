...
export default function Home() {
  ...
  const start = () => { if (!displayName.trim() || !gradeBand || !eventSection) return toast.error("Enter your name, grade group, and Boys or Girls section first."); join.mutate({ displayName, gradeBand, eventSection }); };
  ...
  return <div className="welcome-app"><EventHeader /><main className="container welcome-main" id="welcome">
    <section className="welcome-intro">
      ...
      <section className="join-station" aria-label="Start your TFA quiz record"><div className="join-station__head"><span>YOUR TFA DISCOVERY PASSPORT</span><h2>{accessCode ? "Your ICT route is ready." : "Create your ICT route."}</h2><p>{accessCode ? "Choose any ICT studio. Your completed challenges will stay in your personal record." : "Add the name you would like to celebrate on the live board, then choose your grade group and Boys or Girls section."}</p></div>{accessCode ? <div className="join-station__ready"><BookOpenCheck size={25} /><div><strong>{passport.data?.participant.displayName ?? "Quiz record ready"}</strong><span>{completedCount} of 5 ICT studios completed · {passport.data?.totalPoints ?? 0} points</span></div><Button variant="ghost" onClick={() => setLocation("/passport")}>View record <ArrowRight size={16} /></Button></div> : <div className="join-station__form"><label><span>Display name</span><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Your name" maxLength={80} /></label><label><span>Grade group</span><select value={gradeBand} onChange={event => setGradeBand(event.target.value as typeof gradeBand)}><option value="">Choose grade</option><option value="6-7">Grades 6–7</option><option value="8-9">Grades 8–9</option><option value="10-12">Grades 10–12</option></select></label><label><span>Welcome Day section</span><select value={eventSection} onChange={event => setEventSection(event.target.value as typeof eventSection)}><option value="">Choose section</option><option value="boys">Boys section</option><option value="girls">Girls section</option></select></label><Button className="action-button" onClick={start} disabled={join.isPending}>{join.isPending ? "Starting…" : <>Start ICT route <ArrowRight size={17} /></>}</Button></div>}</section>
    </section>
