  ...
  return <div className="welcome-app"><EventHeader /><main className="container welcome-main" id="welcome">
    <section className="compact-welcome" aria-labelledby="welcome-title">
      <div className="compact-welcome__copy">
        ...
        <h1 id="welcome-title">Welcome to ICT Day.<br /><em>Learn. Create. Play.</em></h1>
        <p>Pick a short challenge, answer three questions, and celebrate what you learn.</p>
        <ol className="welcome-steps" aria-label="How the Welcome Day works"><li><b>1</b><span><strong>Join</strong>Start your record</span></li><li><b>2</b><span><strong>Play</strong>Choose a challenge</span></li><li><b>3</b><span><strong>Celebrate</strong>See your score</span></li></ol>
      ...
      <section className="join-station compact-join" aria-label="Start your TFA quiz record"><div className="join-station__head"><span>START HERE</span><h2>{accessCode ? "You are ready to play." : "Start your ICT day."}</h2><p>{accessCode ? "Your progress is saved. Pick another ICT challenge below." : "Enter your name and grade. Your class code is optional."}</p></div>{accessCode ? <div className="join-station__ready"><BookOpenCheck size={25} /><div><strong>{passport.data?.participant.displayName ?? "Your record is ready"}</strong><span>{completedCount} of 5 ICT challenges complete · {passport.data?.totalPoints ?? 0} points</span></div><Button variant="ghost" onClick={() => setLocation("/passport")}>View my record <ArrowRight size={16} /></Button></div> : <div className="join-station__form"><label><span>Your name</span><Input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Type your name" maxLength={80} /></label><label className="grade-picker"><span>Your grade</span><button type="button" className="grade-picker__trigger" aria-haspopup="listbox" aria-expanded={gradePickerOpen} onClick={() => setGradePickerOpen(open => !open)}><span>{GRADE_GROUPS.find(group => group.value === gradeBand)?.label ?? "Choose your grade"}</span><ChevronDown size={17} /></button>{gradePickerOpen && <span className="grade-picker__options" role="listbox" aria-label="Choose grade group">{GRADE_GROUPS.map(group => <button type="button" role="option" aria-selected={gradeBand === group.value} key={group.value} onClick={() => { setGradeBand(group.value); setGradePickerOpen(false); }}>{group.label}</button>)}</span>}</label><label className="join-station__class-field"><span>Class code <em>optional</em></span><Input value={classLabel} onChange={event => setClassLabel(event.target.value)} placeholder="For example: 7F" maxLength={40} /><small>For example: 7F or 10A. Add Girl/Boy only if useful.</small></label><Button className="action-button" onClick={start} disabled={join.isPending}>{join.isPending ? "Starting…" : <>Start ICT Day <ArrowRight size={17} /></>}</Button></div>}</section>
    </section>
