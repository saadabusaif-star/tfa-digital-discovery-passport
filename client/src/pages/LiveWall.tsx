  ...
  return <div className={`live-app${projectorMode ? " live-app--projector" : ""}`}>
    ...
    <main className={`container live-main${projectorMode ? " live-main--projector" : ""}`}>
      ...
      {board.isLoading ? <div className="live-loading"><span /><p>Opening the live board…</p></div> : <>
        ...
        <section className="live-leaderboard"><header><div><p className="eyebrow"><Trophy size={14} />SUBJECT LEADERS</p><h2>Top result in every lab</h2></div><span>Live scores refresh automatically</span></header><div className="live-leaderboard__grid">{subjectLeaders.map(item => <article key={item.subject} className={`leader-card leader-card--${item.accent}`}><div><span>{item.subject}</span><small>{item.attempts ? `${item.attempts} completed` : "Awaiting a result"}</small></div>{item.leader ? <p><strong>{item.leader.name}</strong><em>{item.leader.score}<small>/3</small> · {item.leader.points} pts</em></p> : <p className="leader-card__empty">Your name could lead this lab.</p>}</article>)}</div></section>
