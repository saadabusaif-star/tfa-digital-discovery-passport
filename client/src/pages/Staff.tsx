...
function StaffContent() {
  ...
  const saveTeacherAccess = trpc.staff.saveTeacherAccess.useMutation({ onSuccess: () => { toast.success("Teacher access saved. The teacher can sign in with this email address."); setTeacherEmail(""); teacherAccess.refetch(); }, onError: error => toast.error(error.message) });
  const setTeacherAccessStatus = trpc.staff.setTeacherAccessStatus.useMutation({ onSuccess: () => { toast.success("Teacher access updated."); teacherAccess.refetch(); }, onError: error => toast.error(error.message) });
  ...
  if (user.role !== "admin" && user.role !== "teacher") return <div className="staff-login"><ShieldCheck size={36} /><h1>Staff access required</h1><p>Your account is signed in, but it has not yet been assigned a teacher or administrator role.</p></div>;
  ...
  return <div className="staff-content">
    ...
    <section className="staff-section"><div className="staff-section__heading"><div><p className="section-kicker">Record & display management</p><h2>Find a student and manage live-board visibility</h2></div><span>Hide or restore names without deleting student records</span></div><div className="resource-fields"><div><Label htmlFor="participant-search">Search this teacher scope</Label><Input id="participant-search" value={participantSearch} onChange={event => setParticipantSearch(event.target.value)} placeholder="Name, grade group, or section" /></div></div><div className="participant-control-list">{visibleParticipants.length ? visibleParticipants.map(participant => <article className="participant-control-row" key={participant.id}><div><h3>{participant.displayName} <small>· Grades {participant.gradeBand} · {participant.eventSection}</small></h3><p>{participant.completedCount}/{participant.activityCount} ICT studios completed · {participant.totalPoints} points</p></div><Button size="sm" variant={participant.isActive ? "outline" : "default"} className={participant.isActive ? "hide-button" : "approve-button"} disabled={setParticipantVisibility.isPending} onClick={() => setParticipantVisibility.mutate({ participantId: participant.id, isActive: !Boolean(participant.isActive) })}>{participant.isActive ? "Hide from live wall" : "Show on live wall"}</Button></article>) : <div className="staff-empty"><UsersRound size={24} /><p>{overview.data?.participants.length ? "No student record matches this search." : "Participant visibility controls will appear here when students join."}</p></div>}</div></section>
